...
### 12.人工智慧必須能夠證明其決定的合理性
DSA 需要明確且具體的理由。 AI 以結構化方式推理：規則 (NG-CONDUCT-004)、偵測到（潛在的具體威脅）、置信度 (0.94)、相關情境（前 4 個訊息）、建議操作（人工審核）。

### 13. 不允許AI偷偷改變內容
**審核人工智慧絕對不能在不被注意的情況下更改原始內容。 ** 自動修正、翻譯或摘要保留原始內容。

### 14.人工智慧產生的內容
我們區分：人類創造、人工智慧輔助、人工智慧生成和人工智慧操縱。這成為內容元資料的一部分。

### 15. AI 內容標籤和 AI 來源層
根據歐盟人工智慧法案的透明度規則（自2026年8月起），人工智慧產生的內容必須具有可辨識性。我們設想一個儲存元資料（AI 起源、模型、時間戳記、人工審核）的 AI 來源層。

### 16. Deepfake 检测
該架構旨在檢測合成影像、受刺激的聲音和深度偽造品。但認可並不能自動證明。

### 17. 沒有自動「真相機器」（審核≠事實檢查）
系統會檢查：“內容是否違反任何規則？” （內容審核），另一個提供：“有哪些資訊和來源？” （資訊援助）。意見並不是單純地因為「錯誤」而被刪除。

### 18. 防止文化誤解
人工智慧需要**文化背景模型**來防止一國的溝通規範被採用為全球標準。

### 19.諷刺、諷刺與幽默
人工智慧使用上下文、表情符號、對話流和眾所周知的諷刺結構，但在含義不清楚時必須考慮不確定性。

### 20. 不會根據單一 AI 分數進行懲罰
任何嚴肅的審核幹預都不能完全基於單一自動分類結果（文本+上下文+行為+語言+媒體+規則引擎=風險評估）。

### 21. 使用者行為訊號與沒有社會信用體系
這是關於技術濫用訊號（例如垃圾郵件群發），而不是一般的社會評級系統。 Nexus Gaja 不運作社會信用體系 - 適度是為了安全，而不是評估一個人的價值。

### 22. 審核人工智慧必須可審計
記錄所有相關的自動決策（事件 ID、規則 ID、置信度、人工審核等）以確保可追溯性。

### 23. 誤報、漏報和品質指標
錯誤類型受到監控。儀表板衡量精確度、召回率，尤其是**上訴逆轉率**（成功上訴的數量）。 

### 24. 語言公平與翻譯偏見
所有受支援語言的審核品質必須具有可比性（多語言審核基準）。如果原文和譯文的審核結果不同（翻譯衝突），則必須單獨檢查。

### 25.架構提案與策略引擎
規則（策略引擎）並未硬編碼到人工智慧模型中。人工智慧提供洞察，政策引擎根據當前規則做出決策。這允許**模型更改而無需更改規則**。

### 26.人民仍然是最終的權威
- **NG-AI-MOD-001**：人工智慧支援檢測和分類，但在做出嚴肅決策時不能取代人工審查。
- **NG-AI-MOD-002**：自動審核決策必須可追蹤、可記錄和可驗證。

**總結**：我們正在建立一個四層系統：人工智慧偵測、背景和風險分析、政策引擎和人類治理。這可以實現強大的自動化，而不會創建危險的「人工智慧作為法官」架構。

## 融資原則與收入模式（WP 1.10.1）

![Nexus Gaja 財務模型](assets/img/nexus_finance.jpg)

一個非常重要的基本經濟原則適用於 Nexus Gaja：**平台內沒有經典廣告。 **
這使得 Nexus Gaja 在概念上不同於當今的許多社交網路。但這並不意味著：Nexus Gaja 不能具有商業特徵。相反：平台必須在經濟上可行，這樣其社會目的才能長期持續存在。因此，經濟活動是達到目的的手段，而不是平台的實際目的。

### 1. 原理 NG-FIN-001
Nexus Gaja 透過獨立於用戶利益的透明收入流為其營運提供資金，而不是透過行銷用戶的注意力或個人資料。
...
這就留下了交流空間：**交流空間而不是廣告空間。 **

### 3. 無需廣告即可融資
相反，我們的融資可以由幾個支柱組成。我們先提出六大支柱：
````文本
                 聯繫加賈
                     │
       ┌──────────────┼──────────────┐
...
#### 第 1 支柱 – 免費基本會員資格
**Nexus Gaja Free：** 國際理解的基本參與不應該取決於某人是否有錢。以下功能可免費使用：個人檔案、國際交流、投稿、評論、基本翻譯、社群、聊天、基本媒體功能。

#### 第二支柱 – 進階優惠
自願付費優惠（**Nexus Gaja Plus**）。可能的好處：更大的儲存限制、更高的媒體品質、更長的視訊傳輸、擴展的翻譯功能、更大的社區、額外的組織功能、更高的人工智慧配額。
**免費增值——但不是“黑暗免費增值”：**免費版本可以合理使用；高級版擴展了它。

#### 支柱 3 – 組織
**Nexus Gaja 組織：** 學校、大學、俱樂部、非政府組織、公司、市政當局等的專用帳戶，具有中央組織帳戶、會員管理和行政功能。 （例如：學校免費或透過機構收費向學生提供存取權限）。

#### 支柱 4 – 捐贈
**Nexus Gaja 資助基金：** 捐贈者可以自願提供資金（一般或專款）。 
**資金池和 tombola：** 免費或折扣使用由捐贈池提供資金。資金分配分類帳確保專款專用。
````文本
捐款 → 用途基金 → 資格引擎 → 免費造訪/折扣/配額
````

#### 支柱 5 – 機構支持
贊助者：基金會、文化資助計畫、國家資助計畫。
**NG-FIN-002（獨立）：** 資金不得購買對 Nexus Gaja 的編輯或技術控制權。

#### 支柱 6 – 商業服務
Nexus Gaja 可以提供 B2B 服務，例如**翻譯即服務 (API)**、專業翻譯、組織溝通或國際會議室，而不會給正常用戶推送帶來廣告負擔。

### 4.沒有數據行銷就沒有“監控經濟”
**NG-FIN-003：** 使用者個人資料不是 Nexus Gaja 的商品。 （不得出售個人資料、歷史記錄、身分資料）。 
Nexus Gaja 無意從對其用戶進行盡可能多的監控和盡可能精確的心理分類中受益。

### 5. 財務透明度
Nexus Gaja 財務透明：可依類別、捐款量、營運成本等公佈收入。 
**資金分類帳：** 指定用於特定目的的捐款以易於理解的會計方式記錄（捐款→基金ID→用途→可用餘額→分配）。沒有透明度就沒有交叉補貼。

### 6. 定價與團結融資模型
價格基於：成本導向、公平、團結。
**團結溢價：** 溢價用戶可以自願選擇：「我資助其他用戶的部分訪問權限。」強制團結或高級社會（對免費用戶較少的尊重或更差的節制）被排除在外。

### 7. 經濟關鍵績效指標
我們不依賴無盡的線上時間（沒有操縱性的參與經濟）。我們的關鍵績效指標：
- **全球溝通指數 (GCI)：** 來自不同語言或文化區域的人們之間有多少成功的溝通關係？
- **平台永續發展比率（PSR）：** 經常性收入/經常性營運成本（目標：PSR ≥ 1）。
...
### 總結：融資原則（NG-FIN）
- **NG-FIN-001：** Nexus Gaja 的資金來源不是傳統廣告。
- **NG-FIN-002：** 財政支援不構成對節製或治理的影響。
- **NG-FIN-003：** 個人使用者資料不是商品。
- **NG-FIN-004：** 基本國際通訊功能應無需付費即可使用。
- **NG-FIN-005：** 高級服務擴大了服務範圍，但不得降低免費用戶的尊嚴和基本通訊選項。
- **NG-FIN-006：** 專用資金是為特定目的而管理的。
- **NG-FIN-007：** 捐贈和資金的管理透明且全面。
- **NG-FIN-008：** 商業服務不得影響平台的獨立性。
- **NG-FIN-009：** Nexus Gaja 的經濟發展應以長期永續發展為目標，而不是以使用者貨幣化最大化為目標。
- **NG-FIN-010：** 經濟結構應永久實現社會目標－國際理解、國際交流和人民之間的友誼。

## API、介面與通訊架構（WP 1.11.3）

為了確保系統穩定性、安全性和可擴展性，Nexus Gaja 遵循嚴格的基於 API 和事件驅動的架構。

### 基本原則
- **無直接資料庫存取：** 元件僅透過定義的介面（API 或事件）進行通信，而從不透過其他服務的直接資料庫查詢進行通信。
- **API 閘道：** 所有外部用戶端請求都透過 API 閘道處理驗證、路由和速率限制。
- **提供者抽象化：** 外部服務（人工智慧模型、支付提供者、翻譯引擎）透過抽象層整合。這可以防止硬連線依賴並允許靈活的提供者切換。

### 溝通模式
- **同步 API (REST/HTTPS)：** 用於即時要求，例如登入、設定檔設定或直接翻譯。
- **非同步事件（事件匯流排）：** Nexus Gaja 的中樞神經系統，用於解耦流程（例如「Message.Created」非同步觸發審核、翻譯和通知）。
- **即時（WebSocket）：** 用於即時聊天和打字狀態的專用通道。

### 安全性和可靠性
- **零信任模型：** 內部網路流量不會自動信任；敏感的服務到服務通訊需要身份驗證。
- **冪等 & 寄件匣模式：** 關鍵操作（例如捐贈或訊息）被設計為冪等，以防止多次處理，並使用寄件匣模式確保即使在資料庫事務中事件也不會遺失。

## MVP 領域模型 (WP 1.12)

![Nexus Gaja 模組化整體](assets/img/nexus_architecture.jpg)

Nexus Gaja 使用嚴格領域驅動的 MVP 架構 (ADR-025)，設計為具有清晰領域邊界的模組化整體。這種結構可以防止早期微服務的複雜性，同時保持日後外包特定領域的彈性。
